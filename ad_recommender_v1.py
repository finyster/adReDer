...
def normalize_age_bin(bin_from_model: str) -> str:
    """
    把模型輸出對齊 JSON 的年齡區間：
    11-20, 21-30, 31-40, 41-50, 51-60, 61-above
    """
...
def create_ad_media_db(base_dir: str = 'ads') -> Dict[str, Dict[str, List[str]]]:
    ...
    for category in sorted([d for d in os.listdir(base_dir) if os.path.isdir(os.path.join(base_dir, d))]):
...
def calculate_ad_scores(ad_data: dict, face_stats: dict,
    ...
    if not physical_scores:
        return 'neutral' if 'neutral' in available_categories else (available_categories[0] if available_categories else None)
...
class AdRecommender:
    ...
    def start_ad(self, category: str):
        ...
        if self.ad_is_video:
            ...
            if self.current_ad_cap.isOpened():
                fps = self.current_ad_cap.get(cv2.CAP_PROP_FPS) or 0.0
                total = self.current_ad_cap.get(cv2.CAP_PROP_FRAME_COUNT) or 0.0
                self.ad_duration = (total / fps) if fps > 1e-3 and total > 0 else 0
            else:
    ...
    def get_ad_frame(self, face_stats: dict, ad_w: int, ad_h: int) -> np.ndarray:
        ...
        is_finished = False
        if self.ad_is_video:
            if ad_frame is None: is_finished = True
        else:
            if self.ad_duration > 0 and elapsed > self.ad_duration: is_finished = True
